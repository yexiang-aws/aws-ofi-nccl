...
exit:
	if (ret_handle != NULL) {
		sendrecv_comm_mr_base_dereg(ret_handle, domain);
	}
	return ret;
}
/*
				       ...
				       nccl_net_ofi_sendrecv_domain_t *domain)
{
	...
	if (nccl_ofi_idpool_active(key_pool) && OFI_UNLIKELY(mr_handle->mr_key != MR_KEY_INIT_VALUE)) {
				       ...
				       nccl_net_ofi_mr_handle_t *mhandle)
{
	...
	printf("sendrecv_recv_comm_dereg_mr\n");
...
static int sendrecv_freelist_regmr_host_fn(void *opaque, void *data, size_t size, void **handle)
{
	...
	printf("sendrecv_freelist_regmr_host_fn\n");
...
static int sendrecv_freelist_deregmr_host_fn(void *handle)
{
	...
	printf("sendrecv_freelist_deregmr_host_fn\n");
				       ...
				       nccl_net_ofi_mr_handle_t *mhandle)
{
	...
	auto *mr_handle = reinterpret_cast<nccl_net_ofi_sendrecv_mr_handle_t *>(mhandle);
	printf("sendrecv_send_comm_dereg_mr\n");
	return sendrecv_comm_mr_base_dereg(mr_handle, domain);
}
