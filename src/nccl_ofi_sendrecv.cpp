					...
					nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	auto *ret_handle = new nccl_net_ofi_sendrecv_mr_handle_t;
	...
	if (nccl_ofi_idpool_active(key_pool)) {
		...
		printf("MR key allocation success\n");
				     ...
				     nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	return sendrecv_mr_buffers_register(domain, ep, dev_id, ckey, type, mr_handle);
}
				       ...
				       nccl_ofi_mr_cache_t *mr_cache)
{
	...
	if (nccl_ofi_idpool_active(key_pool) && OFI_LIKELY(mr_handle->mr_key != MR_KEY_INIT_VALUE)) {
		...
		printf("MR key free success: %d\n", ret);
		printf("MR_KEY_INIT_VALUE: %ld\n", MR_KEY_INIT_VALUE);
		if (OFI_UNLIKELY(ret != 0)) {
	...
	if (mr_handle->mr != NULL) {
		ret = fi_close((fid_t)mr_handle->mr);
		if (OFI_UNLIKELY(ret != 0)) {
				     ...
				     nccl_net_ofi_sendrecv_mr_handle_t **mr_handle)
{
	...
	ret = sendrecv_mr_base_register(domain, ep->ofi_ep,
...
static int sendrecv_recv_comm_close(nccl_net_ofi_recv_comm_t *recv_comm)
{
	...
	printf("sendrecv_recv_comm_close\n");
				   ...
				   nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req_t **base_req)
{
	...
	printf("sendrecv_send_comm_send \n");
...
static int sendrecv_send_comm_close(nccl_net_ofi_send_comm_t *send_comm)
{
	...
	printf("sendrecv_send_comm_close\n");
...
static int nccl_net_ofi_sendrecv_endpoint_free(nccl_net_ofi_ep_t *base_ep)
{
	...
	printf("nccl_net_ofi_sendrecv_endpoint_free\n");
