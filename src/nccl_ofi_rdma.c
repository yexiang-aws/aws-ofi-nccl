				     ...
				     size_t size, int total_ncompls)
{
	...
	if (ncompls == total_ncompls &&
		...
		NCCL_OFI_WARN("DEBUG: total inc_req_completion");
	}
...
static inline int set_send_ctrl_completed(nccl_net_ofi_rdma_req_t *req)
{
	...
	/* Add completion to parent request */
	int ret = inc_req_completion(recv_req, 0, recv_data->total_num_compls);
	
	NCCL_OFI_WARN("DEBUG: set_send_ctrl_completed ret: %d", ret);
	return ret;
}
				 ...
				 nccl_net_ofi_rdma_req_t *req)
{
	...
	NCCL_OFI_WARN("DEBUG: sender received an CTRL");
	if (ctrl_msg->type == NCCL_OFI_RDMA_MSG_CTRL_FI_WRITE) {
		send_data->use_fi_write = true;
		NCCL_OFI_WARN("DEBUG: sender received an early completion CTRL");
	}
	return 0;
}
...
static int process_pending_reqs(nccl_net_ofi_rdma_ep_t *ep)
{
	...
	while (true) {
		...
		NCCL_OFI_WARN("DEBUG: process_pending_reqs: %p, typ: %d", req, req->type);
		switch (req->type) {
			case NCCL_OFI_RDMA_WRITE:
			case NCCL_OFI_RDMA_SEND:
			case NCCL_OFI_RDMA_CTRL_RX_BUFF:
...
static int ofi_process_cq_rail(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail)
{
	...
	while (true) {
		...
		if (rc > 0) {
			...
			// NCCL_OFI_WARN("DEBUG: called fi_cq_read, FI_EAGAIN");
...
static int ofi_process_cq(nccl_net_ofi_rdma_ep_t *ep)
{
	...
	// NCCL_OFI_WARN("DEBUG: in process_pending_reqs, %d", ret);
...
static int test(nccl_net_ofi_req_t *base_req, int *done, int *size)
{
	...
	*done = 0;
	...
	NCCL_OFI_WARN("DEBUG: in test, Request: %s",nccl_net_ofi_req_str(req));
	...
	if (req->state != NCCL_OFI_RDMA_REQ_COMPLETED
		...
		NCCL_OFI_WARN("DEBUG: in test, called ofi_process_cq, returned : %d, req: %p", ret, req);
		if (OFI_UNLIKELY(ret != 0))
			goto exit;
	}
	...
	if (OFI_LIKELY(req->state == NCCL_OFI_RDMA_REQ_COMPLETED)) {
		NCCL_OFI_WARN("DEBUG: in test, NCCL_OFI_RDMA_REQ_COMPLETED");
		...
		*done = 1;
		...
		if (req->type == NCCL_OFI_RDMA_SEND || req->type == NCCL_OFI_RDMA_RECV) {
			...
			if (req->type == NCCL_OFI_RDMA_SEND) {
				NCCL_OFI_WARN("DEBUG: in test, NCCL_OFI_RDMA_SEND");
				msgbuff = ((nccl_net_ofi_rdma_send_comm_t *)base_comm)->msgbuff;
			} else if (req->type ==  NCCL_OFI_RDMA_RECV) {
				NCCL_OFI_WARN("DEBUG: in test, NCCL_OFI_RDMA_RECV");
	...
	// NCCL_OFI_WARN("DEBUG: in test, done nothing");
...
static inline int insert_send_ctrl_req(
				...
				bool is_optional_recv_completion)
{
	nccl_net_ofi_scheduler_t *scheduler = device->scheduler;
	nccl_net_ofi_rdma_ep_t *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
	...
	/* from v9 API, if optional completion is set, then set to NCCL_OFI_RDMA_MSG_CTRL_FI_WRITE */
	ctrl_msg->type = is_optional_recv_completion ? NCCL_OFI_RDMA_MSG_CTRL_FI_WRITE : NCCL_OFI_RDMA_MSG_CTRL;
				...
				bool is_optional_recv_completion)
{
	...
	recv_data->total_num_compls = is_optional_recv_completion ? 1 : 2;
	...
	ret = insert_send_ctrl_req(r_comm, device, dev_id, msg_seq_num, buff, size, buff_mr_handle, req, is_optional_recv_completion);
	if (ret) {
			 ...
			 nccl_net_ofi_req_t **base_req)
{
	...
	bool eager = false;
	bool is_optional_recv_completion = false;

	assert(r_comm != NULL);

	if (ofi_nccl_is_early_completion_enabled() == 1 && *base_req == (void *)NCCL_NET_OPTIONAL_RECV_COMPLETION) {
		is_optional_recv_completion = true;
	}
	...
	ret = allocate_rdma_recv_req(r_comm, device, dev_id, msg_seq_num,
					buffers[0], sizes[0],
					mr_handles[0], &req, is_optional_recv_completion);
	if (ret != 0) {
		goto error;
	}
	...
	*base_req = (nccl_net_ofi_req_t *)req;
	NCCL_OFI_WARN("DEBUG: recv req: %p", req);
					...
					nccl_net_ofi_rdma_req_t **ret_req)
{
	...
	*ret_req = NULL;
	...
	if (eager) {
		...
		/* in the case of early completion, only need the completion for control msg itself */
			   ...
			   bool use_fi_write)
{
	...
	if (use_fi_write) {
		...
		NCCL_OFI_WARN("DEBUG: fi_write: req %p, ep %p ", &req->ctx[rail_id], comm_rail->local_ep);
	} else {
		rc = fi_writedata(comm_rail->local_ep, (void*)((uintptr_t)send_data->buff + xfer_info->offset),
					...
					send_data->remote_buff + xfer_info->offset,
					send_data->remote_mr_key[rail_id], (void *)&req->ctx[rail_id]);
		// NCCL_OFI_WARN("DEBUG: fi_writedata");
	}
	if ((rc != 0) && (rc != -FI_EAGAIN)) {
...
static int send_progress(nccl_net_ofi_rdma_req_t *req)
{
	...
	if (req->type == NCCL_OFI_RDMA_SEND) { // Post RDMA write
		...
		if (send_data->eager) {
			...
			NCCL_OFI_WARN("DEBUG: Eager send");
		} else {
			  ...
			  nccl_net_ofi_rdma_req_t *req)
{
	...
	ssize_t rc = fi_send(comm_rail->local_ep, ctrl_fl_elem->ptr,
			...
			desc,
			     comm_rail->remote_addr, (void *)&req->ctx[rail_id]);
			NCCL_OFI_WARN("DEBUG: CTRL posted");
	if ((rc != 0) && (rc != -FI_EAGAIN)) {
...
retry:
	...
	if (!have_ctrl && (ssize_t)size <= ep->eager_send_size && s_comm->num_inflight_writes == 0) {
		eager = true;
		NCCL_OFI_WARN("DEBUG: eager_max_size: %ld, size: %d \n", ep->eager_send_size, size);
	}
