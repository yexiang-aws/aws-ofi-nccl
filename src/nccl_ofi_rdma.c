				     ...
				     size_t size, int total_ncompls)
{
	...
	if (ncompls == total_ncompls &&
		...
		// NCCL_OFI_WARN("DEBUG: total completion");
	}
...
static inline int set_send_ctrl_completed(nccl_net_ofi_rdma_req_t *req)
{
	...
	req->ncompls = 1;
	req->state = NCCL_OFI_RDMA_REQ_COMPLETED;
	// NCCL_OFI_WARN("DEBUG: NUM CTRL COMPLETIONS");

	nccl_net_ofi_mutex_unlock(&req->req_lock);

	nccl_net_ofi_mutex_lock(&r_comm->ctrl_counter_lock);
	r_comm->n_ctrl_delivered += 1;
	// NCCL_OFI_WARN("DEBUG: CTRL delivered, %lu", r_comm->n_ctrl_delivered);
	...
	for (comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
		...
		if (comp_flags & FI_SEND) {
			...
			if (req->type == NCCL_OFI_RDMA_SEND_CONN || req->type == NCCL_OFI_RDMA_SEND_CONN_RESP) {
				...
				ret = inc_req_completion(req, 0, send_data->total_num_compls);
				NCCL_OFI_WARN("Eager send completion should be 0");
			...
			// NCCL_OFI_WARN("DEBUG: Receive completion processed");
			...
			case NCCL_OFI_RDMA_SEND: {
				...
				NCCL_OFI_WARN("DEBUG: NCCL_OFI_RDMA_SEND completion processed");
				break;
			}
			case NCCL_OFI_RDMA_WRITE: {
			 ...
			 nccl_net_ofi_req_t **base_req)
{
	...
	if (mb_res == NCCL_OFI_MSGBUFF_SUCCESS) {
		...
		if (type == NCCL_OFI_MSGBUFF_REQ) {
			...
			ret = -EINVAL;
			goto error;
		} else if (ofi_nccl_is_eager_mode_enabled() == 1 && OFI_LIKELY(type == NCCL_OFI_MSGBUFF_BUFF)) {
			   ...
			   bool use_fi_write)
{
	...
	if (use_fi_write) {
		...
		// NCCL_OFI_WARN("DEBUG: fi_write posted");
	} else {
		rc = fi_writedata(comm_rail->local_ep, (void*)((uintptr_t)send_data->buff + xfer_info->offset),
					...
					send_data->remote_buff + xfer_info->offset,
					send_data->remote_mr_key[rail_id], req);
		// NCCL_OFI_WARN("DEBUG: fi_writedata posted");
	}

	if ((rc != 0) && (rc != -FI_EAGAIN)) {
		NCCL_OFI_WARN("fi_write failed; RC: %zd, Error: %s",
			      rc, fi_strerror(-rc));
	} else if (rc == 0) {
...
retry:
	...
	eager = false;
	if ((ofi_nccl_is_eager_mode_enabled() == 1) && ((!have_ctrl && (size_t)size <= eager_max_size && s_comm->num_inflight_writes == 0) || (size == 0))) {
		eager = true;
	}
	...
	if (OFI_UNLIKELY(ret != 0 || req == NULL)) {
		NCCL_OFI_WARN("Failed to insert_rdma_send_req_into_msgbuff");
		goto free_req;
	}
